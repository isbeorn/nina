...
namespace NINA.Test.Database {
    ...
    internal class DatabaseInteractionTest {
        ...
        [Test]
        public async Task GetDeepSkyObjects_PreservesEverySortAndLimitWithMissingAliases(
            [Values("id", "ra", "dec", "magnitude", "surfacebrightness", "sizemax", "constellation", "dsotype")] string field,
            [Values("ASC", "DESC")] string direction,
            [Values(0, 2, 3)] int limit) {
            var databasePath = Path.Combine(TestContext.CurrentContext.WorkDirectory, $"dso-sort-{Guid.NewGuid():N}.sqlite");
            try {
                CreateMinimalDsoDatabase(databasePath);
                using (var connection = new SQLiteConnection($"Data Source={databasePath};Pooling=False;")) {
                    ...
                    command.CommandText = @"
                        UPDATE dsodetail SET ra=sizemax, dec=sizemax, magnitude=sizemax, surfacebrightness=sizemax,
                            constellation=CASE id WHEN 'Small' THEN 'AAA' WHEN 'Medium' THEN 'BBB' ELSE 'CCC' END,
                            dsotype=CASE id WHEN 'Small' THEN 'AAA' WHEN 'Medium' THEN 'BBB' ELSE 'CCC' END;
                        DELETE FROM cataloguenr WHERE dsodetailid='Medium';";
                    command.ExecuteNonQuery();
                }
                var interaction = new DatabaseInteraction($"Data Source={databasePath};Pooling=False;");
                var parameters = new DatabaseInteraction.DeepSkyObjectSearchParams {
                    SearchOrder = new DatabaseInteraction.DeepSkyObjectSearchOrder { Field = field, Direction = direction },
                    Limit = limit
                };
                var result = await interaction.GetDeepSkyObjects(null as string, null, parameters, CancellationToken.None);
                IEnumerable<string> expected = field == "id" ? new[] { "Large", "Medium", "Small" } : new[] { "Small", "Medium", "Large" };
                if (direction == "DESC") { expected = expected.Reverse(); }
                result.Select(x => x.Id).Should().Equal(expected.Take(limit));
                if (limit > 1) {
                    var medium = result.Single(x => x.Id == "Medium");
                    medium.Name.Should().Be("Medium");
                    medium.AlsoKnownAs.Should().Equal("Medium");
                }
            } finally {
                SQLiteConnection.ClearAllPools();
                GC.Collect();
