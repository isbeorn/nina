...
namespace NINA.Test.MGEN.Commands {
    ...
    [TestFixture]
    public class StopCameraCommandTest : CommandTestRunner {
        private Mock<IFTDI> ftdiMock = new Mock<IFTDI>();

        [Test]
        public void ConstructorTest() {
            var sut = new StopCameraCommand();

            sut.CommandCode.Should().Be(0xca);
            sut.AcknowledgeCode.Should().Be(0xca);
            sut.SubCommandCode.Should().Be(0xc0);
        ...
        public void Successful_Scenario_Test() {
            SetupWrite(ftdiMock, new byte[] { 0xca }, new byte[] { 0xc0 });
