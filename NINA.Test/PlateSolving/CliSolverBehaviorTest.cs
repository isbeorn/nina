...
namespace NINA.Test.PlateSolving {
    ...
    public class CliSolverBehaviorTest {
        ...
        /// </summary>
        [Test]
        [Explicit("Runs the actual ten-minute solver timeout; execute separately.")]
        public async Task SolveAsync_TimeoutUsesLinkedSolverTimeoutToken() {
            var solver = new TestableCliSolver {
                ShouldSucceed = true,
                ArgumentsToReturn = "/C ping 127.0.0.1 -n 721 > nul"
            };
            ...
            using var fallback = new CancellationTokenSource(TimeSpan.FromMinutes(11));
            PlateSolveResult result = await solver.SolveAsync(imageData, parameter, default, fallback.Token);
            stopwatch.Stop();
            fallback.IsCancellationRequested.Should().BeFalse();

            result.Success.Should().BeFalse();
            solver.ReadResultCalled.Should().BeFalse();
            stopwatch.Elapsed.Should().BeGreaterThan(TimeSpan.FromMinutes(9.9));
            stopwatch.Elapsed.Should().BeLessThan(TimeSpan.FromMinutes(10.5));
        }

        [Test]
        public async Task SolveAsync_CallerCancellationStopsProcessWithoutReadingResult() {
            var solver = new TestableCliSolver { ShouldSucceed = true, ArgumentsToReturn = "/C ping 127.0.0.1 -n 61 > nul" };
            var data = CreateImageData("CallerCancellation", out _);
            using var cancellation = new CancellationTokenSource(TimeSpan.FromMilliseconds(250));
            var stopwatch = System.Diagnostics.Stopwatch.StartNew();
            var result = await solver.SolveAsync(data, new PlateSolveParameter { FocalLength = 600, PixelSize = 3.76 }, default, cancellation.Token);
            result.Success.Should().BeFalse();
            solver.ReadResultCalled.Should().BeFalse();
