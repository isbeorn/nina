using ASCOM.Alpaca.Discovery;
using NINA.Core.Locale;
using NINA.Core.Utility.WindowService;
using NINA.Equipment.Equipment.MySwitch.Ascom;
using NINA.Equipment.Equipment.MyTelescope;
using NINA.Equipment.Interfaces;
using NINA.Profile.Interfaces;
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Linq;
using System.Runtime.CompilerServices;
using System.Text;
using System.Threading;
using System.Threading.Tasks;

namespace NINA.Equipment.Equipment.MyWeatherData {
    internal class AlpacaDirectObservingConditions : IWeatherData, INotifyPropertyChanged {

        private PropertyChangedEventHandler _propertyChanged;
        public event PropertyChangedEventHandler PropertyChanged {
            add => _propertyChanged += value;
            remove => _propertyChanged -= value;
        }

        private AscomObservingConditions device;

        private void SetDevice(AscomObservingConditions newDevice) {
            if (ReferenceEquals(device, newDevice)) return;

            if (device != null)
                device.PropertyChanged -= OnDevicePropertyChanged;

            device = newDevice;

            if (device != null)
                device.PropertyChanged += OnDevicePropertyChanged;
        }

        private void OnDevicePropertyChanged(object sender, PropertyChangedEventArgs e) {
            if (string.IsNullOrEmpty(e.PropertyName) ||
                e.PropertyName == nameof(IDevice.Name) || e.PropertyName == "Name") {
                RaisePropertyChanged(nameof(Name));
            }
            if (string.IsNullOrEmpty(e.PropertyName) ||
                e.PropertyName == nameof(IDevice.DisplayName) || e.PropertyName == "DisplayName") {
                RaisePropertyChanged(nameof(DisplayName));
            }

            RaisePropertyChanged(e.PropertyName);
        }
        private IProfileService profileService;
        private AlpacaDirectSettings settings;

        public AlpacaDirectObservingConditions(IProfileService profileService) {
            this.profileService = profileService;
            settings = new AlpacaDirectSettings(new NINA.Profile.PluginOptionsAccessor(profileService, Guid.Parse(Id)));
        }

        protected void RaisePropertyChanged([CallerMemberName] string propertyName = null) {
            _propertyChanged?.Invoke(this, new PropertyChangedEventArgs(propertyName));
        }

        public bool HasSetupDialog => true;

        public string Id => "6BD8BCE9-C199-401A-AAF8-47EA8EE5AE32";

        public string Name => device?.Name ?? "Alpaca Observing Conditions - Static IP";

        public string DisplayName => (device?.Name ?? "Alpaca Observing Conditions - Static IP") + $" @ {settings.IpAddress} #{settings.DeviceNumber}";

        public string Category => "ASCOM Alpaca";

        public async Task<bool> Connect(CancellationToken token) {
            token.ThrowIfCancellationRequested();
            settings.Validate();
            Disconnect();
            var meta = new AscomDevice();
            meta.UniqueId = Id;
            meta.AscomDeviceName = Name;
            meta.ServiceType = settings.ServiceType;
            meta.IpAddress = settings.IpAddress;
            meta.IpPort = settings.Port;
            meta.AlpacaDeviceNumber = settings.DeviceNumber;
            SetDevice(new AscomObservingConditions(meta));
            var connect = await ((IDevice)device).Connect(token);
            RaisePropertyChanged(nameof(Name));
            RaisePropertyChanged(nameof(DisplayName));
            return connect;
        }

        private IWindowService windowService = new WindowService();

        public void SetupDialog() {
            windowService.OnDialogResultChanged -= WindowService_OnDialogResultChanged;
            windowService.OnDialogResultChanged += WindowService_OnDialogResultChanged;
            windowService.ShowDialog(settings, Loc.Instance["LblAlpacaDirectIPSetup"], System.Windows.ResizeMode.NoResize, System.Windows.WindowStyle.ToolWindow);
        }

        private void WindowService_OnDialogResultChanged(object sender, EventArgs e) {
            windowService.OnDialogResultChanged -= WindowService_OnDialogResultChanged;
            RaisePropertyChanged(nameof(DisplayName));
        }

        public double AveragePeriod => ((IWeatherData)device).AveragePeriod;

        public double CloudCover => ((IWeatherData)device).CloudCover;

        public double DewPoint => ((IWeatherData)device).DewPoint;

        public double Humidity => ((IWeatherData)device).Humidity;

        public double Pressure => ((IWeatherData)device).Pressure;

        public double RainRate => ((IWeatherData)device).RainRate;

        public double SkyBrightness => ((IWeatherData)device).SkyBrightness;

        public double SkyQuality => ((IWeatherData)device).SkyQuality;

        public double SkyTemperature => ((IWeatherData)device).SkyTemperature;

        public double StarFWHM => ((IWeatherData)device).StarFWHM;

        public double Temperature => ((IWeatherData)device).Temperature;

        public double WindDirection => ((IWeatherData)device).WindDirection;

        public double WindGust => ((IWeatherData)device).WindGust;

        public double WindSpeed => ((IWeatherData)device).WindSpeed;

        public bool Connected => ((IDevice)device)?.Connected ?? false;

        public string Description => ((IDevice)device)?.Description ?? string.Empty;

        public string DriverInfo => ((IDevice)device)?.DriverInfo ?? string.Empty;

        public string DriverVersion => ((IDevice)device)?.DriverVersion ?? string.Empty;

        public IList<string> SupportedActions => ((IDevice)device)?.SupportedActions ?? [];

        public void Disconnect() {
            try {
                ((IDevice)device)?.Disconnect();
            } finally {
                SetDevice(null);
            }
        }

        public string Action(string actionName, string actionParameters) {
            return ((IDevice)device).Action(actionName, actionParameters);
        }

        public string SendCommandString(string command, bool raw = true) {
            return ((IDevice)device).SendCommandString(command, raw);
        }

        public bool SendCommandBool(string command, bool raw = true) {
            return ((IDevice)device).SendCommandBool(command, raw);
        }

        public void SendCommandBlind(string command, bool raw = true) {
            ((IDevice)device).SendCommandBlind(command, raw);
        }
    }
}