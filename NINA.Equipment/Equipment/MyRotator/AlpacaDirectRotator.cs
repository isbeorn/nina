...
namespace NINA.Equipment.Equipment.MyRotator {
    internal class AlpacaDirectRotator : IRotator, INotifyPropertyChanged {
        ...
        private AscomRotator device;

        private void SetDevice(AscomRotator newDevice) {
        ...
        public AlpacaDirectRotator(IProfileService profileService) {
        ...
        public string Name => device?.Name ?? "Alpaca Rotator - Static IP";

        public string DisplayName => (device?.Name ?? "Alpaca Rotator - Static IP") + $" @ {settings.IpAddress} #{settings.DeviceNumber}";
        ...
        public async Task<bool> Connect(CancellationToken token) {
            ...
            SetDevice(new AscomRotator(meta));
        ...
        public bool CanReverse => ((IRotator)device).CanReverse;

        public bool Reverse { get => ((IRotator)device).Reverse; set => ((IRotator)device).Reverse = value; }

        public bool IsMoving => ((IRotator)device).IsMoving;

        public bool Synced => ((IRotator)device).Synced;

        public float Position => ((IRotator)device).Position;

        public float MechanicalPosition => ((IRotator)device).MechanicalPosition;
        ...
        public Task<bool> Move(float position, CancellationToken ct) {
            return ((IRotator)device).Move(position, ct);
        }

        public Task<bool> MoveAbsolute(float position, CancellationToken ct) {
            return ((IRotator)device).MoveAbsolute(position, ct);
        }

        public Task<bool> MoveAbsoluteMechanical(float position, CancellationToken ct) {
            return ((IRotator)device).MoveAbsoluteMechanical(position, ct);
        }

        public void Halt() {
            ((IRotator)device).Halt();
        }
