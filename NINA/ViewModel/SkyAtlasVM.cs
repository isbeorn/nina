...
namespace NINA.ViewModel {
    ...
    internal partial class SkyAtlasVM : BaseVM, ISkyAtlasVM {
                ...
                try {
                    ...
                    if (useTransitFilter || OrderByField == SkyAtlasOrderByFieldsEnum.TRANSITTIME) {
                        ...
                        Parallel.ForEach(resultList, dso => {
                            ...
                            DateTime tUpper = NighttimeData.ReferenceDate + TimeSpan.FromHours(hoursToUpperTransit * (23.9344696 / 24.0));
        ...
        private DateTime GetDateFromReferenceDate(DateTime date, DateTime referenceDate) {
            if (date.Hour >= 12) {
                return new DateTime(referenceDate.Year, referenceDate.Month, referenceDate.Day, date.Hour, date.Minute, date.Second, referenceDate.Kind);
            } else {
                return new DateTime(referenceDate.Year, referenceDate.Month, referenceDate.Day, date.Hour, date.Minute, date.Second, referenceDate.Kind) + TimeSpan.FromDays(1);
            }
        }
